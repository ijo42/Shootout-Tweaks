    ...
    public void onRender(RenderGameOverlayEvent event) {
        ...
        if (currentDamageReceived > 0) {
            if (receivedUpdate + TimeUnit.SECONDS.toMillis(durationDisplay) < currentTimeMillis) {
                ...
                GL11.glPushMatrix();
                fontRenderer
                        .drawStringWithShadow(String.format("%.2f", currentDamageReceived), width / 2 + 5, height / 2, receivedDamageColor);
                GL11.glPopMatrix();
            }
        }
        ...
        if (currentDamageTaken > 0) {
            if (takenUpdate + TimeUnit.SECONDS.toMillis(durationDisplay) < currentTimeMillis) {
                ...
                GL11.glPushMatrix();
                fontRenderer
                        .drawStringWithShadow(String.format("%.2f", currentDamageTaken), width / 2 - 5, height / 2 + 10, takeDamageColor);
                GL11.glPopMatrix();
            }
        }
